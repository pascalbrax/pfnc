//! Placeholder for the macOS `pfnc-agent` — the SSH-deployed, QUIC-speaking
...
//! on a non-macOS dev machine never needs to compile it. It's still a
//! normal standalone crate — `cargo build -p pfnc-agent-macos` works fine
//! since this stub has no platform-specific code yet.

fn main() {
    println!("pfnc-agent (macOS): not yet implemented — see crates/agent-linux for the reference agent.");
}
